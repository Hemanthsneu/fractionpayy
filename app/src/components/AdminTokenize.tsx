...
export function AdminTokenize() {
  ...
  return (
    <div className="mx-auto max-w-lg rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
      ...
      <div className="mt-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={form.assetType} onChange={(e) => set("assetType", e.target.value)} className="mt-1 w-full rounded-lg border border-white/10 bg-slate-900 px-3 py-2 text-sm outline-none">
      ...
      <button onClick={tokenize} disabled={busy} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-400 to-cyan-400 py-3 font-semibold text-black transition hover:opacity-90 disabled:opacity-40">
        {busy ? <Loader2 size={16} className="animate-spin" /> : <Factory size={16} />}
        {busy ? "Deploying on Arc…" : "Tokenize & list"}
      </button>
      {busy && statusStep && <p className="mt-2 text-center text-xs text-cyan-300">{statusStep}</p>}

      {result && (
        <div className="mt-4 rounded-xl border border-emerald-300/20 bg-emerald-400/10 p-4 text-sm text-emerald-300">
          ...
          <Link href="/marketplace" className="mt-3 inline-flex items-center gap-1 text-xs text-cyan-300 hover:underline">
...
function ExplorerRow({ label, kind, value }: { label: string; kind: "address" | "tx"; value: string }) {
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-1.5 transition hover:bg-black/30">
      ...
      <span className="flex items-center gap-1 font-mono text-[11px] text-cyan-300">{shortHash(value)} <ExternalLink size={10} /></span>
    </a>
  );
}
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm outline-none focus:border-emerald-300/40" />
    </div>
  );
}
